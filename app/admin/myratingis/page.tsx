...
export default async function MyratingisDashboard() {
  const data = await fetchProjectStats("myratingis");
    ...
    <div className="space-y-8">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[11px] font-black tracking-[0.2em] uppercase text-[var(--admin-text-muted)]">마이레이팅이즈</span>
        </div>
        <h1 className="text-2xl font-bold text-[var(--admin-text)]">대시보드</h1>
        <p className="text-sm text-[var(--admin-text-muted)] mt-1">myratingis.com</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "총 사용자", value: stats?.totalUsers ?? 0, color: "text-slate-600" },
          { label: "월간 활성", value: stats?.mau ?? 0, color: "text-orange-600" },
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link
            href="/admin/myratingis/users"
            className="group flex items-start gap-4 p-5 bg-[var(--admin-card)] rounded-xl border border-[var(--admin-border)] hover:border-orange-200 hover:shadow-sm transition-all"
          >
            <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 bg-orange-50">
              <Users className="w-5 h-5 text-orange-600" />
            </div>
            <div>
              <p className="text-sm font-bold text-[var(--admin-text)] group-hover:text-orange-600 transition-colors">사용자 관리</p>
